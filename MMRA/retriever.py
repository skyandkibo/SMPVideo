...
def retrieve_items(mode, k, train_path, valid_path, test_path):
    ...
    if mode == 'test':
        ...
        if valid_path is not None:
            ...
            database_df = pd.concat([train_df], axis=0)
...
def stack_retrieved_feature(train_path, valid_path, test_path):
    ...
    if valid_path is not None:
        ...
        df_database = pd.concat([df_train, df_test], axis=0)
...
if __name__ == "__main__":
    ...
    train_path = f'{args.input_dir}/train.pkl'

    if args.model == 'train_val':
        valid_path = f'{args.input_dir}/valid.pkl'
    else:
        valid_path = None

    test_path = f'{args.input_dir}/test.pkl'
    ...
    if args.model == 'train_val':
        ...
        stack_retrieved_feature(train_path, None, test_path)
